// =============================================================================
// render/renderer.rs — camera (zoom/pan) + state vertex attribute
// =============================================================================
...
// Matches RenderParams in render.wgsl (16 bytes)
#[repr(C)]
...
impl Renderer {
    pub async fn new(window: Arc<Window>, num_particles: u32) -> Self {
        ...
        let render_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("Render Bind Group"), layout: &render_bgl,
            entries: &[wgpu::BindGroupEntry {
                binding: 0, resource: render_buf.as_entire_binding(),
            }],
        });
        ...
        Self {
            surface, device, queue, config, size, render_pipeline, num_particles,
            render_params, render_buf, render_bind_group,
        }
    }
    ...
    // =========================================================================
    // Camera controls — called from main.rs event handlers
    // =========================================================================

    pub fn zoom_at(&mut self, delta: f32, cursor_x: f32, cursor_y: f32) {
        // delta > 0 = zoom in, delta < 0 = zoom out
        // We zoom toward the cursor position so the point under the cursor stays fixed.
        let factor = if delta > 0.0 { 1.15f32 } else { 1.0 / 1.15 };

        // Convert cursor from pixel coords to world coords (before zoom)
        let w = self.size.width  as f32;
        let h = self.size.height as f32;
        // Cursor in normalised device coords [-1, 1]
        let ndc_x = (cursor_x / w) * 2.0 - 1.0;
        let ndc_y = 1.0 - (cursor_y / h) * 2.0;
        // World coords = ndc * aspect / zoom - pan
        let world_x = ndc_x * self.render_params.aspect / self.render_params.zoom
                    - self.render_params.pan_x;
        let world_y = ndc_y / self.render_params.zoom - self.render_params.pan_y;

        self.render_params.zoom *= factor;
        self.render_params.zoom  = self.render_params.zoom.clamp(0.1, 50.0);

        // Adjust pan so the world point under the cursor stays under the cursor
        // new_ndc = (world + pan_new) * zoom_new / aspect  => solve for pan_new
        self.render_params.pan_x = ndc_x * self.render_params.aspect / self.render_params.zoom - world_x;
    ...
    pub fn pan(&mut self, dx: f32, dy: f32) {
        // dx, dy in pixels; convert to world-space delta
        let w = self.size.width  as f32;
        let h = self.size.height as f32;
        // 2.0 / (zoom * screen_size) converts pixels to world units
        self.render_params.pan_x += dx * 2.0 * self.render_params.aspect / (w * self.render_params.zoom);
    ...
    fn upload_render_params(&self) {
        self.queue.write_buffer(&self.render_buf, 0, bytemuck::bytes_of(&self.render_params));
    }
