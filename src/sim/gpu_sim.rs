...
impl GpuSim {
    ...
    fn make_physics_bg(
        ...
        device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry {
                    binding: 4,
                    resource: hash.sorted_indices.as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 5,
                    resource: hash.cell_start.as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 6,
                    resource: hash.cell_counts.as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 7,
                    resource: state_transfer.as_entire_binding(),
                },
            ],
        })
    }
    ...
    pub fn tick(&self, encoder: &mut wgpu::CommandEncoder) {
        ...
        self.hash
            .build_with_assign_bg(encoder, &self.hash_assign_bgs[self.frame_idx % 2]);
...
fn generate_particles(n: u32, config: &InitConfig) -> Vec<GpuParticle> {
    ...
    match config {
        ...
        InitConfig::Clustered { n_clusters } => {
            ...
            let cluster_centers: Vec<[f32; 2]> = (0..nc)
                .map(|i| {
        ...
        InitConfig::Rings => {
            ...
            (0..n)
                .map(|i| {
                    ...
                    make_particle(angle.cos() * radius, angle.sin() * radius, t, 0.5)
                })
                .collect()
        }
        ...
        InitConfig::Grid => {
            ...
            (0..n)
                .map(|i| {
                    ...
                    make_particle(x, y, col % NUM_TYPES, 0.5)
                })
                .collect()
        }
    }
